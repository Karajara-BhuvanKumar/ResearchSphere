...
const PostdocPositions = () => {
  ...
  const {
    data: postdocPositions = [],
    isLoading,
    error,
  } = useQuery({
    queryKey: ["postdoc-positions", debouncedQuery],
    queryFn: () =>
      searchHarvestedData({
        kind: "opportunity",
        subtype: "postdoc",
  ...
  const sourceOptions = useMemo(() => {
    const unique = new Set(
      postdocPositions.map((item) => getCanonicalSourceLabel(item)).filter(Boolean),
    );
    return Array.from(unique).sort((a, b) => String(a).localeCompare(String(b)));
  }, [postdocPositions]);

  const processedPostdocs = useMemo(() => {
    ...
    const filtered = postdocPositions.filter((item) => {
      if (sourceFilter !== "all") {
  ...
  }, [postdocPositions, sourceFilter, sortBy]);

  const totalPages = Math.max(1, Math.ceil(processedPostdocs.length / itemsPerPage));
  const currentPostdocs = processedPostdocs.slice(
    ...
    <div className="min-h-screen flex flex-col bg-[hsl(var(--background))]">
      ...
      <main className="flex-1">
        <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-emerald-50/60 shadow-[0_24px_70px_-50px_rgba(15,23,42,0.45)]">
            <div className="border-b border-slate-200/80 px-5 py-5 sm:px-8 sm:py-7">
              ...
              <div className="mt-4 max-w-3xl">
                <h1 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-[2.1rem]">
                  Track postdoctoral openings across labs, departments, and research centers
                </h1>
                <p className="mt-3 text-[15px] leading-7 text-slate-600 sm:text-base">
                  Search postdoc roles in computing, AI, and interdisciplinary research with
                  a presentation that feels aligned with ResearchSphere, while staying distinct
                  from funding and call-for-pages workflows.
                </p>
              </div>
            </div>
                ...
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.55fr)_240px_220px]">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        placeholder="Search by specialization, lab, institution, or research theme"
                        value={searchQuery}
          ...
          <section className="mt-8">
            <div className="flex flex-col gap-2 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-medium text-slate-500">Research career opportunities</p>
                <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">
                  {processedPostdocs.length} postdoc positions found
                </h2>
              </div>
              <p className="text-sm text-slate-500">
                Global job boards and institution-led listings in one streamlined view.
              </p>
            </div>

            {isLoading ? (
              <div className="py-16 text-center text-muted-foreground">
                Loading postdoc opportunities...
              </div>
            ) : error ? (
              <div className="py-16 text-center text-red-500">
                Error loading postdoc opportunities. Please try again later.
              </div>
            ) : processedPostdocs.length === 0 ? (
              <div className="py-16 text-center text-muted-foreground text-lg">
                No postdoc positions found matching "{searchQuery}"
              </div>
            ) : (
              <div className="mt-6 space-y-5">
                {currentPostdocs.map((position: HarvestItem) => {
                  const sourceLabel = getCanonicalSourceLabel(position);
                  const postedDate = formatDisplayDate(position.eventDate);
                  const deadline = formatDisplayDate(position.deadline) || position.deadline;

                  return (
                    <Card
                      key={position.id}
                      className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_18px_48px_-38px_rgba(15,23,42,0.4)] transition-all duration-200 hover:border-emerald-200 hover:shadow-[0_24px_60px_-42px_rgba(16,185,129,0.35)]"
                    >
                      <CardContent className="p-0">
                        <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_280px]">
                          <div className="p-5 sm:p-6">
                            <div className="flex flex-wrap items-center gap-2">
                              <Badge className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-50">
                                Postdoc position
                              </Badge>
                              <Badge
                                variant="secondary"
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-600">
                              <span className="inline-flex items-center gap-2">
                                ...
                                {position.organization || sourceLabel}
                              </span>
                              {postedDate && (
                                <span className="inline-flex items-center gap-2">
                            ...
                            <div className="flex h-full flex-col justify-between gap-5">
                              <div>
                                ...
                                <h4 className="mt-4 text-base font-semibold text-slate-950">
                                  Review the postdoctoral listing before applying
                                </h4>
                                <p className="mt-2 text-sm leading-6 text-slate-600">
                                  Confirm research area fit, funding terms, application
                                  materials, and lab expectations directly from the source page.
                                </p>
                              </div>
                              ...
                              >
                                <a href={position.url} target="_blank" rel="noopener noreferrer">
                                  View position <ExternalLink className="ml-1 h-4 w-4" />
                                </a>
                              </Button>
                            </div>
                          </aside>
...
export default PostdocPositions;
